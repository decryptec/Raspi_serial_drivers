...
static int adxl345_probe(struct spi_device *spi) {
        ...
        if (ret) {
                dev_err(&spi->dev, "Failed  to start measurment\n");
                return ret;
        }

        data_format = 0x0B;
        ret = write_reg(spi, REG_DATA_FORMAT, data_format);
        if (ret) {
                dev_err(&spi->dev, "Failed to set data format\n");
                return ret;
        }

        ret = write_reg(spi, REG_BW_RATE, 0x0A); // Set 100Hz sampling
        if (ret) {
                dev_err(&spi->dev, "Failed to set sampling rate\n");
                return ret;
        }
